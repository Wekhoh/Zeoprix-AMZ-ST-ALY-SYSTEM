...
def _bootstrap_backend(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv(
        ...
        f"sqlite:///{(tmp_path / 'backend-workspace.db').as_posix()}",
    )
...
def test_admin_can_create_member_and_new_member_can_log_in(monkeypatch, tmp_path):
    ...
    with TestClient(create_app()) as client:
        ...
        workspace_response = client.get(
            "/workspaces/default", headers={"Authorization": f"Bearer {owner_token}"}
        )
        ...
        teammate_token, teammate = _login(
            client, "teammate@example.com", "teammate-password"
        )
        assert teammate["role"] == "editor"

        teammate_workspace = client.get(
            "/workspaces/default", headers={"Authorization": f"Bearer {teammate_token}"}
        )
...
def test_member_endpoint_protects_last_admin_role(monkeypatch, tmp_path):
    ...
    assert (
        "At least one workspace admin must remain assigned."
        in demote_response.json()["detail"]
    )
