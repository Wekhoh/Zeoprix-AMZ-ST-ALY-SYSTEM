...
def _bootstrap(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('AMZ_BACKEND_DATABASE_URL', f"sqlite:///{(tmp_path / 'backend-frontend-mutations.db').as_posix()}")
    ...
    db.upsert_manual_review(product_id=product_id, term='travel pillow', term_type='keyword', campaign_id=campaign_id, relevance='pending', reviewed=False)
    db.close()
...
def test_frontend_actions_batch_endpoints_roundtrip(monkeypatch, tmp_path):
    ...
    with TestClient(create_app()) as client:
        create_response = client.post('/frontend/actions/execution-batches', json={'product_id': product_id, 'batch_type': 'negative', 'draft_note': 'from test'})
        assert create_response.status_code == 200
        batch = create_response.json()
        assert batch['batch_type'] == 'negative'
        assert batch['id']

        update_response = client.patch(f"/frontend/actions/execution-batches/{batch['id']}", json={'status': 'executed', 'execution_note': 'done'})
...
def test_frontend_review_decision_endpoint_updates_manual_review(monkeypatch, tmp_path):
    ...
    with TestClient(create_app()) as client:
        response = client.post('/frontend/review/manual-reviews', json={
