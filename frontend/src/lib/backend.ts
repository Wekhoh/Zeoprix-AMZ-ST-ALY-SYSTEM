...
export async function getWorkbenchPayload(productId?: number): Promise<WorkbenchPayload> {
  ...
  try {
    return mergeWorkbench(await getJson<Partial<WorkbenchPayload>>(`/frontend/workbench${params}`))
  } catch {
    return mockWorkbenchPayload
  }
}

export async function getUploadPayload(productId?: number): Promise<UploadPayload> {
  const params = productId ? `?product_id=${productId}` : ""
  try {
    return mergeWorkbench(await getJson<UploadPayload>(`/frontend/upload${params}`)) as UploadPayload
  } catch {
    return mockWorkbenchPayload as UploadPayload
  }
}

export async function getAnalysisPayload(productId?: number): Promise<AnalysisPayload> {
  const params = productId ? `?product_id=${productId}` : ""
  try {
    return mergeWorkbench(await getJson<AnalysisPayload>(`/frontend/analysis${params}`)) as AnalysisPayload
  } catch {
    return mockWorkbenchPayload as AnalysisPayload
  }
}

export async function getActionsPayload(productId?: number): Promise<ActionsPayload> {
  const params = productId ? `?product_id=${productId}` : ""
  try {
    return mergeWorkbench(await getJson<ActionsPayload>(`/frontend/actions${params}`)) as ActionsPayload
  } catch {
    return mockWorkbenchPayload as ActionsPayload
  }
}

export async function getReviewPayload(productId?: number): Promise<ReviewPayload> {
  const params = productId ? `?product_id=${productId}` : ""
  try {
    return mergeWorkbench(await getJson<ReviewPayload>(`/frontend/review${params}`)) as ReviewPayload
  } catch {
    return mockWorkbenchPayload as ReviewPayload
  }
}

export async function getSettingsPayload(productId?: number): Promise<SettingsPayload> {
  const params = productId ? `?product_id=${productId}` : ""
  try {
    return mergeWorkbench(await getJson<SettingsPayload>(`/frontend/settings${params}`)) as SettingsPayload
  } catch {
