...
export function SettingsMutationPanel({
	...
	async function clearRuntime() {
		...
		startTransition(async () => {
			const response = await fetch(
				`${BACKEND_BASE_URL}/frontend/settings/clear-runtime`,
				{
					method: "POST",
	...
	async function downloadBackup() {
		...
		startTransition(async () => {
			const response = await fetch(
				`${BACKEND_BASE_URL}/frontend/settings/full-backup?product_id=${productId}`,
			);
			...
			const blob = new Blob([body.content], {
				type: body.mime ?? "application/json",
			});
	...
	async function restoreBackup(file: File) {
		...
		startTransition(async () => {
			const response = await fetch(
				`${BACKEND_BASE_URL}/frontend/settings/restore-backup`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
			...
			const body = (await response
				.json()
				.catch(() => ({ detail: "恢复失败" }))) as RestoreBackupResponse & {
				detail?: string;
			};
			if (!response.ok) {
			...
			<label className="mt-4 flex cursor-pointer flex-col gap-3 rounded-2xl bg-zinc-50 p-4 text-sm text-zinc-500">
				...
				<div className="flex items-center gap-2">
					<input
						...
						onChange={(e) => setRestoreAsNew(e.target.checked)}
						type="checkbox"
					/>
					<span>恢复为新产品副本</span>
				</div>
