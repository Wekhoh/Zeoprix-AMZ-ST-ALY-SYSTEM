      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-[260px_minmax(0,1fr)_320px]">
        <aside className="border-r border-black/6 bg-white px-6 py-6">
          <div className="mb-10">
            ...
            <h1 className="font-display text-[1.65rem] leading-[1.05] font-semibold tracking-[-0.04em] text-[#111111]">Zeoprix Ops Workbench</h1>
            <p className="mt-4 max-w-[18rem] text-[14px] leading-7 text-black/48">把搜索词分析、执行批次、复盘与 AI 副驾驶收进同一个运营工作台。</p>
          </div>
          ...
          <div className="mb-8 rounded-[24px] border border-black/8 bg-[#fafaf7] p-4">
            ...
            <div className="mt-3 text-[15px] font-medium text-[#111111]">{productContext.workspace}</div>
            <div className="mt-1 text-[14px] text-black/50">{productContext.name}</div>
          ...
          <div className="mt-8 rounded-[24px] border border-black/8 bg-[#f6f7f4] p-4">
            ...
            <div className="mt-2 text-[14px] font-medium text-[#111111]">{productContext.lastBackupAt}</div>
            <p className="mt-3 text-[13px] leading-6 text-black/48">建议切换新类目前，先导出完整备份并标记恢复点。</p>
          </div>
        </aside>
          ...
          <div className="relative z-10">
            <header className="mb-10 flex items-start justify-between gap-8 border-b border-black/6 pb-8">
              <div className="max-w-4xl">
                ...
                <h2 className="mt-4 max-w-[11ch] font-display text-[3.75rem] leading-[0.94] font-semibold tracking-[-0.06em] text-[#111111]">{title}</h2>
                <p className="mt-5 max-w-3xl text-[17px] leading-8 text-black/50">{subtitle}</p>
              </div>
              <div className="rounded-[28px] border border-black/8 bg-white px-5 py-4 text-right shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
                ...
                <div className="mt-3 max-w-[14rem] text-[14px] leading-6 font-medium text-[#111111]">先止损，再补量，最后复盘分歧词</div>
              </div>
            </header>
            {children}
          </div>
        ...
        <aside className="bg-[#f6f7f4] px-6 py-6">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[15px] font-medium text-[#111111]">AI 运营副驾驶</div>
            </div>
          </div>
          ...
          <div className="mb-4 rounded-[24px] border border-black/8 bg-white p-4">
            ...
            <div className="mt-2 text-[14px] font-medium text-[#111111]">{productContext.name}</div>
            <p className="mt-2 text-[13px] leading-6 text-black/48">基于最近一次分析结果、执行批次与审核沉淀生成建议。</p>
          </div>
          ...
          <div className="space-y-4">
            {aiCopilotCards.map((card) => (
              <section key={card.title} className="rounded-[24px] border border-black/8 bg-white p-4">
                ...
                <div className="mt-3 text-[15px] leading-6 font-medium text-[#111111]">{card.summary}</div>
                <p className="mt-2 text-[13px] leading-6 text-black/48">{card.context}</p>
                <div className="mt-4 flex flex-wrap gap-2">
          ...
          <div className="mt-6 rounded-[24px] border border-black/8 bg-white p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="mt-1 text-[13px] text-black/50">用副驾驶把结果解释成可执行动作</div>
              </div>
              <Settings2 className="h-4 w-4 text-black/35" />
            </div>
            <div className="mt-4 rounded-[20px] border border-black/8 bg-[#fafaf7] p-3 text-[13px] text-black/45">“帮我基于当前批次，生成老板摘要和执行备注”</div>
            <button className="mt-4 w-full rounded-[20px] bg-[#111111] px-4 py-3 text-[14px] font-medium text-white transition hover:bg-black">打开 AI 助手</button>
          </div>
        </aside>
      </div>
    </div>
