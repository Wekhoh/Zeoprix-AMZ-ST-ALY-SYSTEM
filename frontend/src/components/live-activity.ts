...
export function readFrontendActivity(productId: number | null | undefined): RecentActivityItem[] {
...
export function recordFrontendActivity(productId: number | null | undefined, item: RecentActivityItem) {
  if (typeof window === "undefined") return
  try {
    const raw = window.localStorage.getItem(storageKey(productId))
