...
export function ActionsMutationPanel({
	payload,
	backendBaseUrl,
	onBatchCreated,
	onBatchUpdated,
	...
	async function createBatch(batchType: string) {
		...
		startTransition(async () => {
			const response = await fetch(
				`${backendBaseUrl}/frontend/actions/execution-batches`,
				{
					method: "POST",
			...
			const body = (await response
				.json()
				.catch(() => ({}))) as BatchMutationResponse;
			setMessage(
				batchType === "negative" ? "否词执行批次已生成" : "手动投放批次已生成",
			);
	...
	async function advanceBatch(status: "executed" | "reviewed") {
		...
		startTransition(async () => {
			const response = await fetch(
				`${backendBaseUrl}/frontend/actions/execution-batches/${latestBatch.id}`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						status,
						execution_note:
							status === "executed" ? note || undefined : undefined,
						review_note: status === "reviewed" ? note || undefined : undefined,
					}),
				},
			);
			if (!response.ok) {
			...
			const body = (await response
				...
				.catch(() => ({}))) as BatchMutationResponse;
			setMessage(
				status === "executed" ? "批次已标记为已执行" : "批次已标记为已复盘",
			);
			onBatchUpdated?.(body);
			setNote("");
		});
	...
	return (
		<section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
			...
			<div className="mt-4 flex flex-wrap gap-3">
				...
				<button
					disabled={pending || !productId}
					onClick={() => createBatch("manual")}
					className="rounded-full border border-zinc-200 bg-white px-5 py-2.5 text-sm font-medium text-zinc-900 shadow-sm transition hover:bg-zinc-50 disabled:opacity-50"
				>
					生成手动批次
				</button>
				<button
					disabled={pending || !latestBatch}
					onClick={() => advanceBatch("executed")}
					className="rounded-full border border-zinc-200 bg-white px-5 py-2.5 text-sm font-medium text-zinc-900 shadow-sm transition hover:bg-zinc-50 disabled:opacity-50"
				>
					标记最近批次已执行
				</button>
