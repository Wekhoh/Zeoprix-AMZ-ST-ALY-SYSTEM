...
- 2026-04-17: 继续补强新设置页：新增规则版本恢复入口，前端可直接把某个配置版本恢复回当前产品，进一步减少回到旧 Streamlit 的必要。

- 2026-04-17: 新设置页继续逼近旧系统替代：新增规则版本差异预览与恢复入口，最近版本可直接查看新增/移除项并恢复。

- 2026-04-18: 修复桌面快捷方式启动旧版的问题：保留 .lnk 指向 start.bat，但把 start.bat/start.ps1 改成新版工作台启动器，固定拉起 FastAPI 8008 + Next 3031 并打开新版前端。
- 2026-04-20: 修复桌面快捷方式与新版入口切换：桌面 .lnk 已改回 jackl 路径，start.bat 改为优先调用 pwsh，start.ps1 直接拉起 FastAPI 8008 + Next 3031；同时移除 next/font/google 依赖，避免桌面启动时因 Google Fonts 拉取失败而打不开新版前端。
- 2026-04-20: Actions 页继续产品化：执行批次开始暴露 itemsDetail，展开后可查看词、动作、动作类型、花费、销售，不再只停在 preview 卡片。
